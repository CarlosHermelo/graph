...
evaluate_question_prompt = PromptTemplate(
    ...
    Evaluar la pregunta: Determina si la pregunta es de carácter general o temática (específica). 
    
    Para clasificarla considera si la pregunta puede responderse con una visión global (general) o si requiere detalles específicos que pueden encontrarse en fragmentos de varios documentos (temática).    
...
# Invocar la función buscar_similitud y pasar el nuevo parámetro "tipo"
   # query = "¿Hay algun plan integral oncologico??"
    resultados = buscar_similitud(query, tipo)
    print("-----------------#####Busqueda Por fragmento campo RESULTADOS #######-----------------------")
    print( resultados)
    print("--------------------#####--------------------")
...
# Procesar el resultado para agregarlo a state["retrieved_data"]
    ...
    state["retrieved_data"] = resultados
    print("-----------------#####Busqueda Por fragmento campo STATE #######-----------------------")
    print(state["retrieved_data"])
    print("--------------------#####--------------------")
...
# Nodo 4: Generar la respuesta final usando un LLM
def generate_response(state: State):
    try:
        # Obtenemos la pregunta mejorada y la información recuperada desde el estado
        improved_question = state["improved_question"]
        retrieved_data = state["retrieved_data"]

        # Definir el sistema de mensajes para establecer el rol del LLM
        system_message = """
        Eres un asistente experto en temas legales que trabaja dentro de la obra social PAMI. Tu objetivo es ayudar a otros abogados o empleados de la organización a comprender y comunicar mejor la información recuperada.
        Debes presentar la información de manera clara, estructurada y bien justificada, asegurándote de que la respuesta sea útil y completa.
        Si la pregunta no puede ser respondida con el contexto proporcionado pero el contexto  tiene algun dato de la pregunta  debera mencionarse, indicando el codigo de la resolucion de la metadata que corresponde a ese fragmento.
    
te paso este mensaje que tiene una pregunta y un contexo. Debes responder en base a la pregunta usando el contexto unicamente. La respuesta debe basarse unicamente en el contexto . Si no encuentra la respuesta en el contexto debe decir que no la encuentra.
El contexto tiene una serie de fragmentos y Cada fragmento tiene la siguientes estructura: Fragmento , un Content y una metadata.  
Con estos fragmento debes elaborar la respuesta .
Con los fragmentos que utilices para la respuesta toma de su metadata el codigo que dice en su Fragmento de la resolución, por ejemplo de aquie Fragmento de la resolución: RESOL-2024-2568-INSSJP-DE#INSSJP su codigo es  RESOL-2024-2568-INSSJP-DE#INSSJP y ponelo como referencia en la respuesta.
La pregunta es: 
        
        """

        # Crear la estructura de los mensajes que se van a enviar al modelo LLM
        messages = [
            {"role": "system", "content": system_message},
            {"role": "user", "content": f"Pregunta: {improved_question}"},
            {"role": "user", "content": f"Contexto: {retrieved_data}"}
        ]

        print("-----------------######Genero la  RESPUESTA ######-----------------------")
        print(state["retrieved_data"])
        print("--------------------#####--------------------")

        # Ejecutamos el LLM con los mensajes definidos
        output = llm.invoke(messages)

        # Procesar la respuesta del LLM
        if isinstance(output, AIMessage):
            response_message = output.content
        else:
        ...
        # En caso de error, registrar el mensaje de error y devolver el estado sin cambios significativos
        print(f"Error inesperado en generate_response: {e}")
        state["messages"].append(("assistant", "Hubo un error al generar la respuesta."))
...
if __name__ == "__main__":
    ...
    pregunta = "¿ que dice la resolucion RESOL-2024-2563-INSSJP-DE#INSSJP ?"
    pregunta = "¿ que trantan las resoluciones RESOL-2024-2568-INSSJP-DE#INSSJP y RESOL-2024-2563-INSSJP-DE#INSSJP?"
    pregunta = "¿ Incremento de aranceles ?"
    pregunta = "¿ Que resoluciones tratan chequeate en casa  ?" # no encuentra respuesta
   
    pregunta = "¿resoluciones Estructura orgánico-funcional de las UGL?" # ESTA es ok 
    pregunta = "¿Anexo I de la RESOL-2024-2431-INSSJP-DE#INSSJP?" # esta ok
 # esta ok
    pregunta = "¿resoluciones Estructura orgánico-funcional de las UGL?" # rara respuesta
    
    
    pregunta = "¿Que dice la resolucion RESOL-2024-2537-INSSJP-DE#INSSJP?" 
   
    pregunta = "¿Disposición 7339/GPSyC/13?" 
    pregunta = "¿Que dice Ley N° 27.360?" # esta ok
    pregunta = "¿Alguna resoucion menciona el Decreto PEN N° 2/04?" # 
    pregunta = "Que resoluciones tratan sobre Incremento de aranceles para CLUBES Y CENTROS DE DIA"
